...
def VGS(d,N,n,tau_p,tau_s,gamma_po,gamma_so,rho_w,kw):
    d,N,n,tau_p,tau_s,gamma_po,gamma_so
    ug = grainSize(N)
    f = np.logspace(1,4,num=1000) # in Hz
    ...
    return f, cp, cs, alpha_p, alpha_s


def VGSdf(parameters):
    (T,S,dw,cw,N,d,n,tau_p,tau_s,gamma_po,gamma_so) = (
                                                    parameters['T'],
                                                    parameters['S'],
                                                    parameters['dw'],
                                                    parameters['cw'],
    ...
    f, cp, cs, alpha_p, alpha_s = VGS(d, N, n, tau_p, tau_s, gamma_po, gamma_so, rho_w, kw)
    
    return pd.DataFrame({'f':f,'cp':cp,'cs':cs,'alpha_p':alpha_p,'alpha_s':alpha_s})
